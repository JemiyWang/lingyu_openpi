...
def get_config(variant: Variant) -> Config:
    ...
    if variant == "gemma_2b_lora":
        return Config(
            ...
            num_kv_heads=1,
            head_dim=256,
            lora_configs={"attn": lora.LoRAConfig(rank=16, alpha=16.0), "ffn": lora.LoRAConfig(rank=16, alpha=16.0)},  # rank 从 16 减到 4 以节省显存
        )
    if variant == "gemma_300m_lora":
        ...
        return Config(
            ...
            num_kv_heads=1,
            head_dim=256,
            lora_configs={"attn": lora.LoRAConfig(rank=32, alpha=32.0), "ffn": lora.LoRAConfig(rank=32, alpha=32.0)},  # rank 从 32 减到 4 以节省显存
        )
