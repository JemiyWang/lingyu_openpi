...
class TrainConfig:
    ...
    num_workers: int = 32
    # Number of batches queued per data-loader worker. Larger values hide video-decoding latency.
    prefetch_factor: int = 2
    # Keep CPU batches in page-locked memory before transferring them to accelerators.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(
            ...
            action_dim=32,  # Teleavatar uses 16-dim actions
             # 添加 soft inpainting 参数
            use_soft_inpainting=False,
            time_threshold_inpaint=0.3,
            use_correlated_noise=False,  # 如需启用，先在步骤1加 --compute-correlation
    ...
    TrainConfig(
        ...
        # repo_id: "/mnt/mnt/data/lingyu/pick_paper_20fps"
        # "/mnt/mnt/data/lingyu/build_blocks_20fps"
        # "/mnt/mnt/data/lingyu/task2_put_trash/put_trash_20fps"
        # /mnt/mnt/data/lingyu/lingyu_datasets/expert_pick_food
        data=LeRobotTeleavatarDataConfig(
            repo_id="/mnt/mnt/data/lingyu/lingyu_datasets/expert_pick_food",  # Pick paper dataset 
            base_config=DataConfig(
                prompt_from_task=True,  #
                action_sequence_keys=("action",)  # Use 'action' not 'actions'
            ),
        ...
        # weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        weight_loader=weight_loaders.CheckpointWeightLoader("/mnt/mnt/data/behavior/checkpoints/pi0_base/params"),
        batch_size=64,  # Must be divisible by number of devices (8)
        num_train_steps=20000,
        num_workers=32,
    ),

    TrainConfig(
        name="pi0_teleavatar_norm_stats_compute",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0_config.Pi0Config(
            action_dim=32,  # Keep 32 to match pi0_base pretrained weights
            action_horizon=30,
        ...
        # /mnt/mnt/data/lingyu/lingyu_datasets/expert_pick_food
        data=LeRobotTeleavatarDataConfig(
            repo_id="/mnt/mnt/data/lingyu/lingyu_datasets/pick_paper_all_merged",  # Pick paper dataset
            base_config=DataConfig(
                prompt_from_task=True,  #
                action_sequence_keys=("action",)  # Use 'action' not 'actions'
            ),
    ...
    TrainConfig(
        name="pi0_teleavatar_lora",
        # LoRA fine-tuning for low memory GPU (16GB)
        model=pi0_config.Pi0Config(
            paligemma_variant="gemma_2b_lora",  # 使用 LoRA 减少显存
            action_expert_variant="gemma_300m_lora",  # 使用 LoRA 减少显存
            action_dim=32,  # Keep 32 to match pi0_base pretrained weights
            action_horizon=5,  # 减少到 10 以降低显存需求
            # 添加 soft inpainting 参数
        ...
        data=LeRobotTeleavatarDataConfig(
            repo_id="placebluetoy_datasets2",  # Your local dataset name
            base_config=DataConfig(
                prompt_from_task=True,
                action_sequence_keys=("action",)  # Use 'action' not 'actions'
            ),
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_base/params"),
        batch_size=1,  # 16GB GPU 需要较小的 batch size
        num_train_steps=20000,
        num_workers=1,
        # 冻结非 LoRA 参数
        freeze_filter=pi0_config.Pi0Config(
